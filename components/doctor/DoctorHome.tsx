...
import { mockAppointments, mockBills, mockPrescriptions } from '@/lib/mock-data';

interface DoctorHomeProps { onNavigate: (tab: string) => void; }

export default function DoctorHome({ onNavigate }: DoctorHomeProps) {
    const todayAppointments = mockAppointments.filter(a => a.status === 'upcoming');
    ...
    const totalRevenue = mockBills.filter(b => b.status === 'Paid').reduce((s, b) => s + b.total, 0);
    ...
    const stats = [
        ...
        { icon: Pill, label: 'Prescriptions', value: String(mockPrescriptions.length), color: '#16a34a', bg: '#dcfce7', trend: '' },
    ];
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                    ...
                    <View>
                        ...
                        <Text style={{ fontSize: 22, fontWeight: '700', color: '#0284c7' }}>Dr. Sameer Mahadik</Text>
                    </View>
                </View>
